...
function renderMovies(movies) {
  favoriteMovie = getMovieFromLocalStorage();
  ...
  movies.map((input) => {
    ...
    listItem.innerHTML += `
         <img class="poster" alt="movie-title" src=${
         ...
         <section class="vote-favouriteIcon">
           ...
           <i class="favorite-icon fa-regular fa-heart fa-2xl ${
             favoriteMovie.includes(input.title) ? "fa-solid" : null
           }" id="${input.title}"></i>
         </section>`;

    const favouriteButton = listItem.querySelector(".favorite-icon");
...
function addMovieToLocalStorage(movieName) {
  ...
  localStorage.setItem(
    "favouriteMovies",
    JSON.stringify([...existingMovie, movieName])
  );
}
